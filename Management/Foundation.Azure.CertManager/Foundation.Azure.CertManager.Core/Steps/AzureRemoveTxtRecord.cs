...
{
    public AzureRemoveTxtRecord(IConfiguration config)
        : base(config) { }

    public async Task<Result> ExecuteAsync(Domain domain)
    {
        await CreateClient().DnsZones
            .GetByResourceGroup(domain.ResourceGroup, domain.Name)
            .Update()
